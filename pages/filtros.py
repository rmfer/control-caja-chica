...
st.title("Control de Cajas Chicas 2025 - Filtros")

# Filtros sidebar
cajas = st.sidebar.multiselect("Caja", options=sorted(df_mov["Caja"].unique()), default=sorted(df_mov["Caja"].unique()))
proveedores = st.sidebar.multiselect(
    "Proveedor",
    options=sorted(df_mov[df_mov["Caja"].isin(cajas)]["Proveedor"].dropna().unique()),
    default=sorted(df_mov[df_mov["Caja"].isin(cajas)]["Proveedor"].dropna().unique())
)
areas = st.sidebar.multiselect(
    "Área",
    options=sorted({area for sublist in df_mov[df_mov["Caja"].isin(cajas)]["Área"] for area in sublist if area}),
    default=sorted({area for sublist in df_mov[df_mov["Caja"].isin(cajas)]["Área"] for area in sublist if area})
)
...
# Gráfico consumo por proveedor (si solo una caja)
if len(cajas) == 1 and not df_filtrado.empty:
    ...
    fig.update_layout(width=800, height=400, margin=dict(l=40, r=40, t=40, b=40), dragmode=False,
                      xaxis=dict(fixedrange=True), yaxis=dict(fixedrange=True),
                      xaxis_title="Proveedor", yaxis_title="Consumo")
    st.plotly_chart(fig, use_container_width=False)

    # Tabla movimientos filtrados
